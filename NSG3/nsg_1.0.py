...
class Player:
    def __init__(self):
        ...
        self.altimate = 0
        self.altpoint = 0
        self.atcard = np.zeros((2,3))  #1は物理、２は魔法
        self.reforce = np.zeros((2,3)) #0 ~ 3でプラス強化するステータス値を選択する。
        self.atpower = [0,0]
    ...
    def card_draw(self):
        for stat in range(3):
            self.atcard[0][stat] = random.randint(1,2)
...
def turn_end(target):
    ...
    if selectc[0] != -1 and selectc[1] != -1:
        ...
        player[turn].attack(player[target])
        selectc[0] = -1
        selectc[1] = -1
...
def gameflow(prog):
    ...
    if prog == 0:
        ...
        draw_button(screen,0,150,500,50,"HP:"+str(player[turn].hp)+"  ATK:"+str(player[turn].atstat)+"  MAGIC:"+str(player[turn].magicstat)+"  DEF:"+str(player[turn].defstat)+"  MAGIC_DEF:"+str(player[turn].magicdefstat),"gray",(60,50,60))
        ...
        if selectc[0] >= 0 and selectc[1] >= 0  and target != -1:
